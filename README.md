...
15 分鐘與 1 小時的 TD 訊號卡另提供「空方動能確認」。它依使用者提供的 AI Momentum [YinYang] 預設參數，以 TD 訊號**前 5 根已完成 K 棒**檢查是否出現空方動能，並要求在 TD 前 60 根 K 棒內已先收盤跌破趨勢帶外緣；TD 出現時仍必須位於帶外下方、低於黃色中線，且收盤不高於當時跌破 K 棒。可用「僅空方動能確認」篩選；此為研究輔助條件，不是買賣建議。
