...
另有「**趨勢帶死亡交叉後站回白線**」監測，適用 15 分鐘與 1 小時。白線為 AI Momentum 的 `kernClose`，黃線為 `zoneMid`。條件為：收盤在 Trend Trader EMA 50／EMA 100 趨勢帶下方時，白線下穿黃線；接著在 30 根完成 K 棒內，收盤由下往上站回白線，且白線仍在黃線下方。只有最近 5 根完成 K 棒內的回站訊號會顯示；字卡的橘點標示死亡交叉、藍點標示回站訊號。
