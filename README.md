...
另有「**做多｜趨勢帶下方死亡交叉後站回白線**」監測，適用 15 分鐘與 1 小時。白線為 AI Momentum 的 `kernClose`，黃線為 `zoneMid`。此模組**只產生做多回站訊號**：死亡交叉當根的收盤、白線與黃線必須全部位於 Trend Trader EMA 50／EMA 100 趨勢帶下方，白線再下穿黃線；其後 30 根完成 K 棒內，收盤由下往上站回白線，且白線仍在黃線下方。只有最近 5 根完成 K 棒內的回站訊號會顯示；字卡的橘點標示死亡交叉、藍點標示回站訊號。
