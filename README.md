...
首頁頂端另有「全球期貨脈搏」與「指標股盤前／夜盤異動」：顯示台指期、NASDAQ 100、道瓊、SOX、Russell 2000、日經 225 與南韓 KOSPI 200 期貨的最近報價及漲跌幅。每張期貨卡以脈衝點、動態入場效果與當日 1 分鐘走勢迷你圖強調行情；每次掃描取最新 1 分鐘 K 線，但 GitHub Actions 最快仍約每 5 分鐘發布一次，並非逐筆報價。南韓 KOSPI 200 期貨若公開資料來源無法提供連續期貨報價，會明確標示為 KOSPI 200 現貨指數替代報價。另於美東 04:00–09:30 掃描 70 檔大型指標股的盤前異常漲跌；同時自動從 Binance USDⓈ-M 的免費公開 API 發現並監控你 200 檔美股清單中可交易的 USDT 股票永續合約（目前直接對應 45 檔）。這些是衍生性合約、不是 NASDAQ 現股；異動優先相對各股最近一般盤收盤計算，若該資料暫缺才明確改用 Binance 24 小時開盤作基準。沒有 Binance 合約的股票則繼續使用原本的正式盤前資料來源。預設異常門檻為 `±2%`，最多顯示 12 檔；可在 workflow 的 `PREMARKET_ABNORMAL_PCT` 與 `PREMARKET_MAX_MOVERS` 調整。
...
這個模組只顯示 Setup 9 與 Countdown 13，並保留最近 **5 根已完成 K 棒**（含最新一根）內出現的訊號；超過 5 根會自動排除。網站可篩選美股、台股、幣安現貨、買方、賣方，並在每個週期內依訊號出現時間排序。訊號字卡顯示產業類型、精確產生時間與最近 30 根完成 K 棒走勢，圓點標示 TD 訊號位置；台股訊號也會顯示證券代號與中文名稱，例如 `2330 台積電`。

15 分鐘與 1 小時的**賣方 TD**訊號卡另提供「空方動能確認」。它依使用者提供的 AI Momentum [YinYang] 預設參數，以 TD 訊號**前 5 根已完成 K 棒**檢查是否出現空方動能；並在 TD 當根及其前 29 根完成 K 棒中，確認 AI Momentum 的黃色中線曾位於 Trend Trader EMA 50／EMA 100 趨勢帶的下緣（0.1% 容許差）或下方，且該根黃線斜率向下。買方 TD 不會取得空方動能確認。可用「僅空方動能確認」篩選；此為研究輔助條件，不是買賣建議。

另有「**做多｜趨勢帶下方死亡交叉後站回白線**」監測，適用 15 分鐘與 1 小時。白線為 AI Momentum 的 `kernClose`，黃線為 `zoneMid`。此模組**只產生做多回站訊號**：從死亡交叉到回站的每一根完成 K 棒，收盤、白線與黃線都必須位於 Trend Trader EMA 50／EMA 100 趨勢帶下方；任一根重回趨勢帶即作廢。接著在 30 根完成 K 棒內，收盤由下往上站回白線，且白線仍在黃線下方。只有最近 5 根完成 K 棒內的回站訊號會顯示；字卡的橘點標示死亡交叉、藍點標示回站訊號。
...
- **美股**：`symbols.csv` 的 200 檔流動性較高股票，涵蓋科技、金融、醫療、消費、工業、能源與通訊等產業；訊號卡會顯示產業分類。
- **台股**：每次執行時讀取台股監測清單，並以其上市／上櫃現貨標的計算；產業類型另由上市／上櫃公司公開基本資料帶入。ETF 不列入此區塊。
