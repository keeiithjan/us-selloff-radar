...
- 首頁最上方的日／週線白線、橙線站回雷達：可切換日線與週線。只要先前已有黑 K 實體由線上跌到線下，且當根開盤高於同一條線的開盤基準，即於市場交易期間列入左側開盤站回；休市後不保留舊開盤事件。右側「已確認站回第一根」只會在該週期收盤確認後，把首次實體完全站回或紅 K 實體穿回的標的加入名單，不會把盤中暫時站回當成已確認，也不要求當根開盤在線上；下一根日 K 或週 K 開始形成時，仍保留最近一根已收盤的確認結果。每張字卡分別顯示訊號 K 棒時間與該字卡第一次出現在網站的台北時間；首次顯示時間會依 `signal_id` 跨 GitHub Actions 掃描保留，不會隨網站更新改變。頂部可依週期（日／週）、線別（全部／白線／橙線）及市場（全部／台股／美股／其他）篩選，並提供頁面／PWA 開啟期間的瀏覽器通知去重。日線、週線各有一鍵 TradingView TXT 輸出，會沿用目前線別與市場篩選，合併左右兩側訊號並自動去重。
- 大黑 K 跌破／貼近白線雷達：日線搜尋最近 3 個已完成交易日，週線搜尋最近 3 根已完成週 K。黑 K 開收實體跌幅必須至少 5%，下引線占整根高低區間嚴格小於 10%；同一根實體由白線上方穿越到收盤位於白線下方，或雖未穿越但收盤距 AI Momentum 白線在 ±1% 內，皆會列入。另可啟用「金叉 < 50 根」篩選，只留下訊號當根位於白線向上穿越黃線後第 0～49 根的標的；字卡會固定記錄訊號當根的金叉距離，不會隨後續 K 棒改寫。可切換日／週、白黃線狀態及全部／台股／美股／其他市場，所有篩選會同步套用到日線、週線 TradingView TXT 輸出。
...
首頁站回雷達的「一鍵輸出日線清單」及「一鍵輸出週線清單」會把目前線別／市場篩選中的開盤站回與已確認第一根站回合併去重，每行只輸出一個 `交易所:代號`，可直接匯入 TradingView。

大黑 K 雷達的日線／週線一鍵輸出會沿用該區的市場及「金叉 < 50 根」篩選，輸出最近 3 根已完成 K 棒內符合「實體跌幅至少 5%、實體跌破白線或收盤距白線 ±1%、下引線占整根小於 10%」的標的；同一標的只保留一列。
...
- `tradingview/KJ_Big_Black_White_Golden_Screener.pine`：完整大黑 K Screener。同一份腳本由 Pine Screener 的「指標時間週期」選日線或週線；可分別篩選全部近 3 根候選及訊號當根位於白黃金叉後第 0～49 根的候選。
...
GitHub Actions 平日每 5 分鐘更新一次，讓台股 09:00 與美股 09:30 開盤後的站回訊號優先進入下一輪部署。行情資料主要來自 Yahoo Finance（透過 `yfinance`）、台灣證交所／櫃買中心公開公司基本資料、台灣期交所標的清單，以及 Binance 公開 K 線與成交資料。Pepperstone 商品池依其公開 CFD 商品建置；由於 CFD 沒有集中式成交量，流動性以對應期貨／現貨指數的市場深度作為代理，非 Pepperstone 自身成交量。
