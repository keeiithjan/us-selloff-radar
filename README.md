...
- 首頁最上方的日／週線白線、橙線站回雷達：可切換日線與週線。只要先前已有黑 K 實體由線上跌到線下，且當根開盤高於同一條線的開盤基準，即於市場交易期間列入左側開盤站回；休市後不保留舊開盤事件。右側「已確認站回第一根」只會在該週期收盤確認後，把首次實體完全站回或紅 K 實體穿回的標的加入名單，不會把盤中暫時站回當成已確認，也不要求當根開盤在線上；下一根日 K 或週 K 開始形成時，仍保留最近一根已收盤的確認結果。每張字卡分別顯示訊號 K 棒時間與該輪資料部署至網站的台北時間。頂部可依週期（日／週）、線別（全部／白線／橙線）及市場（全部／台股／美股／其他）篩選，並提供頁面／PWA 開啟期間的瀏覽器通知去重。日線、週線各有一鍵 TradingView TXT 輸出，會沿用目前線別與市場篩選，合併左右兩側訊號並自動去重。
