...
首頁頂端另有「全球期貨脈搏」與「指標股盤前／夜盤異動」：顯示台指期、NASDAQ 100、道瓊、SOX、Russell 2000、日經 225 與南韓 KOSPI 200 期貨的最近報價及漲跌幅。每張期貨卡以脈衝點、動態入場效果與當日 1 分鐘走勢迷你圖強調行情；每次掃描取最新 1 分鐘 K 線，但 GitHub Actions 最快仍約每 5 分鐘發布一次，並非逐筆報價。南韓 KOSPI 200 期貨若公開資料來源無法提供連續期貨報價，會明確標示為 KOSPI 200 現貨指數替代報價。另於美東 04:00–09:30 掃描 70 檔大型指標股的盤前異常漲跌；不論美股是否開盤，都額外以 Binance USDⓈ-M 的免費公開 API 監控 `AMDUSDT` 股票永續合約。AMDUSDT 是衍生性合約、不是 NASDAQ 現股；優先以 AMD 最近一般盤收盤計算異動，若該資料暫缺則明確改用 Binance 24 小時開盤作基準。預設異常門檻為 `±2%`，最多顯示 12 檔；可在 workflow 的 `PREMARKET_ABNORMAL_PCT` 與 `PREMARKET_MAX_MOVERS` 調整。
